          ...
          >
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {sampleMemes.map((meme, i) => (
                <motion.div
                  key={i}
                  className="card bg-gradient-to-br from-white/10 to-white/5 p-4 float-animation cursor-pointer hover:scale-105 transition-transform"
                  style={{ animationDelay: `${i * 0.2}s` }}
                >
                  <div className="w-full h-32 bg-gradient-to-br from-primary-500/20 to-accent-500/20 rounded-lg flex items-center justify-center relative overflow-hidden">
                    <div className="absolute inset-0 bg-gradient-to-br from-purple-600/30 to-blue-600/30" />
                    <div className="relative text-center">
                      <span className="text-3xl mb-2 block">{meme.emoji}</span>
                      <p className="text-white text-xs font-bold px-2">{meme.text}</p>
                    </div>
                  </div>
                  <p className="text-sm text-gray-300 mt-2 font-semibold">{meme.title}</p>
